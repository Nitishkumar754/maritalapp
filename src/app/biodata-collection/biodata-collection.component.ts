...
getCampagin(){
	this.common.commonService({},"GET", "biodata/campaignStatus")
    .subscribe((data:any)=>{
...
listBioData(){
	this.common.commonService({},"GET", "biodata/listBioData")
    .subscribe((data:any)=>{
      console.log(data);
...
isValidForm(formData){
	const errors = [];
	const mandatoryFields = ['name', 'mobileNumber', 'gender', 'biodata','photo1', 'photo2', ''];
	formData.forEach((value, key) => {
		if(!value){
			if(key == 'name') errors.push(`अपलोडकर्ता का नाम भरना अनिवार्य है! `);
			if(key == 'mobileNumber') errors.push(`मोबाइल नंबर भरना अनिवार्य है! `);
...
uploadBiodata(){
  ...
  this.common.uploadFileService(formData, "biodata/uploadBiodata")
    .subscribe((data:any)=>{
