...
export class ForgotPasswordComponent implements OnInit {
  ...
  send_password_reset_otp(){
    ...
    this.common.commonService({email:this.emailForm.form.value.email}, "POST", "user/update/password/otp")
    .subscribe((data:any)=>{
  ...
  validate_password(pass, pass_re){
    if(pass.length<6){
      this.password_error_message = "password must be atleast 6 characters long";
      return false;
    }
  ...
  update_password(){
    ...
    this.common.commonService(request, "POST", `user/update/password/request`)
    .subscribe((data:any)=>{
      console.log("data ", data);
