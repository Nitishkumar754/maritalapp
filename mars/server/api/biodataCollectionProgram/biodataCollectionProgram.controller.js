...
module.exports.uploadBiodataApi = async(req, res) => {
	console.log("request.connection.remoteAddress ********", req.headers['x-forwarded-for'] ||
	...
	if(!commondata.length){
		return res.status(400).send({"message":"यह अभियान (campaign) समाप्त हो गया है"})
	}
	...
	for(let i=0;i < req.files.length;i++){
		...
		if(sizeinMb > 5){
			return res.status(200).send({"message":"फाइल साइज 5mb से अधिक की अनुमति नहीं है "});
			break;
		}
		const uploadParams = {
			content: file.buffer,
	...
	try{
		...
		BioddataCollection.create({
			...
			uploadedBy: name,
			bioDataUrl: result[0].status=='success'? result[0].path : undefined,
...
module.exports.getCampaignStatus = async(req, res)=>{
    ...
    if(!campaignStatus) termsandconditions = '';
	let campaignObj = {isActive, endTime, campaignStatus, maleBioAmount, femaleBioAmount, termsandconditions};
